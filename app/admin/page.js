...
export default function AdminPage() {
  ...
  const handleFullCheckAndSave = async (onlyTest = false) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (!onlyTest) {
          await fetch('/api/settings', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
              routerHost, routerPort, routerUser, routerPass, 
              endpoint: data.publicEndpoint || externalIp
            })
          });
      ...
      setLogs(prev => [...prev, "❌ Error crítico de comunicación."]);
    } finally {
      setIsTesting(false);
    }
  };
  ...
  const handleDeleteNetwork = async (networkName) => {
    ...
    setLogs(["🗑️ Intentando borrar red..."]);
    try {
      ...
      if (data.success) {
        setLogs(prev => [...prev, `✅ Red ${networkName} borrada con éxito.`]);
        fetchNetworks(true);
      } else {
        alert("❌ Error: " + data.error);
        setLogs(prev => [...prev, `❌ Error al borrar: ${data.error}`]);
      }
    } catch (e) {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '2rem auto', fontFamily: 'sans-serif', padding: '0 20px' }}>
      ...
      <section style={{...secStyle, background: 'white'}}>
        ...
        <button onClick={handleCreate} style={btnPri}>🚀 Crear Interfaz en Router</button>
      </section>
      ...
      <section style={{...secStyle, padding: 0, overflow: 'hidden', background: 'white'}}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead style={{ background: '#f8fafc' }}>
            <tr><th style={thStyle}>Nombre</th><th style={thStyle}>Red</th><th style={thStyle}>Acción</th></tr>
          </thead>
          <tbody>
              ...
              <tr key={net.id} style={{ borderTop: '1px solid #e2e8f0' }}>
                ...
                <td style={tdStyle}>
                  {net.comment === 'mikroguard' ? (
                    <button onClick={() => handleDeleteNetwork(net.name)} style={{...btnSec, background: '#ef4444'}}>
                      🗑️ Borrar
                    </button>
                  ) : (
